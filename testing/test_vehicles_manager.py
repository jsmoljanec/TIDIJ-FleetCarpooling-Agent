...
class VehiclesManagerTest(unittest.TestCase):
    ...
    def test_get_all_vehicle_data_from_invalid_vehicle_id_returns_no_vehicle_data(self, mock_db_reference,
                                                                                  mock_initialize_app):
        fake_data = None
    ...
    def test_get_all_vehicle_data_from_valid_vehicle_id_returns_vehicle_data(self, mock_db_reference,
                                                                             mock_initialize_app):
        fake_data = {'active': 'true', 'brand': 'Audi', 'capacity': '5', 'registration': 'VZ152DF',
                     'fuelConsumption': 8, 'model': 'A4 Avant'}
        mock_db_reference.return_value.get.return_value = fake_data
        mock_initialize_app.return_value = MagicMock()
        firebase_manager = FirebaseAdminManager(firebase_credentials_path, firebase_database_url)
        firebase_manager.vehicles_manager.get_all_vehicle_data = MagicMock(return_value=fake_data)
    ...
    def test_get_vehicle_nominal_fuel_consumption_from_valid_vehicle_id_returns_nominal_vehicle_fuel_consumption(self,
                                                                                                                 mock_db_reference,
                                                                                                                 mock_initialize_app):
        fake_data = 8
        mock_db_reference.return_value.get.return_value = fake_data
    ...
    def test_get_vehicle_traveled_distance_from_valid_vehicle_id_returns_vehicle_traveled_distance(self,
                                                                                                   mock_db_reference,
                                                                                                   mock_initialize_app):
        fake_data = 20000
        mock_db_reference.return_value.get.return_value = fake_data
